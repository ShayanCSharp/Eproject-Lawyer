...
<head>
    ...
    <title>Practice Area || Legal Link</title>
...
<body>
    ...
    <div class="page-wrapper">
        ...
        <?php if(isset($_GET['slug'])){$slug=mysqli_real_escape_string($conn,$_GET['slug']);$sql=mysqli_query($conn,"SELECT id, name FROM `practiceareas` WHERE slug = '$slug'");$practiceAreaId=0;$name='';while($row=mysqli_fetch_assoc($sql)){$practiceAreaId=$row['id'];$name=$row['name'];};$sql1=mysqli_query($conn,"SELECT l.first_name, l.id, l.aboutLawyer, l.email, l.picture, l.mobile_num, l.last_name, p.name AS expertise
            FROM lawyers AS l
            INNER JOIN practiceareas AS p ON l.expertise = p.id
            WHERE l.expertise = $practiceAreaId and l.approve = 1");if(mysqli_num_rows($sql1)>0){ ?>
                <section class="inner-banner inner-banner__overlay-two" style="background-image: url(assets/images/background/practice-area.webp); background-size: cover;">
                    <div class="container">
                ...
                <section class="team-two">
                    <div class="container py-5">
                        <div class="block-title text-left">
                            <h3 class="block-title__title">Lorem Ipsum</h3><!-- /.block-title__title -->
                        ...
                        <div class="law-cont d-flex ">
                            <?php while($row1=mysqli_fetch_assoc($sql1)){ ?>
                                <div class="lawyer">
                                    <div class="team-two__single">
                                        ...
                                        <div class="team-two__content px-3">
                                            <a href="lawyer-profile.php?id=<?=$row1['id']?>" class="team-two__name"><?=$row1['first_name']?> &nbsp; <?=$row1['last_name']?></><!-- /.team-two__name -->
        ...
        <?php }else{include("assets/inc/nothing-found.php");}}else{include("assets/inc/nothing-found.php");} ?>
